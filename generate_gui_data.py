...
def main():
    ...
    for (client, session), rows in sorted(groups.items()):
        ...
        sellers_seen = set()
        for r in rows:
            if r["author"] in SELLERS:
                sellers_seen.add(r["author"])

        if sellers_seen:
            seller_label = next(iter(sellers_seen))
        else:
            seller_label = "Comercios Unidos"
        ...
        total_valence = 0.0
        has_low_valence = False
        has_excluded = False
        for m in messages:
            total_valence += m["valence"]
            if m["valence"] <= -0.4:
                has_low_valence = True
            if "correctores" in m["text"].lower() or "archivo" in m["text"].lower():
                has_excluded = True
        avg_v = round(total_valence / len(messages), 4) if messages else 0.0
        ...
        conversations.append({
            ...
            "hasAlert": has_low_valence and not has_excluded,
